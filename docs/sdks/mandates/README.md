...
func main() {
    ...
    res, err := s.Mandates.Get(ctx, "cst_5B8cwPMGnU", "mdt_5B8cwPMGnU", client.Bool(false))
    if err != nil {
        log.Fatal(err)
    }
    if res.MandateResponse != nil {
...
func main() {
    ...
    res, err := s.Mandates.Revoke(ctx, "cst_5B8cwPMGnU", "mdt_5B8cwPMGnU", &operations.RevokeMandateRequestBody{
        Testmode: client.Bool(false),
    })
    if err != nil {
        log.Fatal(err)
